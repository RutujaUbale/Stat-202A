...
def mySVM(X, Y, Xtest, Ytest, m=100, num_iterations=4000,
    ...
    for it in xrange(int(num_iterations)):
        ...
        c = np.repeat(np.multiply(db,Y),p, axis=1)
        dbeta = np.dot(np.ones((1,n)),(np.multiply(c,X1/n)))
...
beta, train_accuracy, testing_accuracy = mySVM(training_digits, training_labels, testing_digits, testing_labels)
